...
# Define the connection string and table name in the backend
DB_URL = "postgresql+psycopg2://brioadmin:Gbsm%401234@briopgdb.postgres.database.azure.com:5432/postgres"
TABLE_NAME = "sfcdata"
...
class ConversationMemory:
    def __init__(self, max_tokens=12000):
        ...
        self.chat_history = []
        self.chat_sessions = {}  # To save multiple chat sessions
    ...
    def save_current_chat(self, name):
        """Save the current chat session."""
        self.chat_sessions[name] = self.chat_history.copy()

    def load_chat(self, name):
        """Load a saved chat session."""
        if name in self.chat_sessions:
            self.chat_history = self.chat_sessions[name].copy()
            # Ensure messages are stored in a consistent format
            self.messages = [
                {
                    "role": msg.role if isinstance(msg, ChatMessage) else msg["role"],
                    "content": msg.content if isinstance(msg, ChatMessage) else msg["content"]
                }
                for msg in self.chat_history
            ]
            # Recalculate tokens
            self.current_tokens = sum(num_tokens_from_string(msg["content"]) for msg in self.messages)
...
def main():
    ...
    with st.sidebar:
        ...
        # Save Chat Button
        chat_name = st.text_input("Save Current Chat As", placeholder="Enter chat name")
        if st.button("Save Chat"):
            if chat_name:
                st.session_state.conversation_memory.save_current_chat(chat_name)
                st.success(f"Chat '{chat_name}' saved!")
            else:
                st.error("Please enter a chat name to save.")

        # Load Chat Dropdown
        saved_chats = list(st.session_state.conversation_memory.chat_sessions.keys())
        if saved_chats:
            selected_chat = st.selectbox("Load Saved Chat", saved_chats)
            if st.button("Load Chat"):
                st.session_state.conversation_memory.load_chat(selected_chat)
    ...
    if st.session_state.data is not None:
        ...
        with st.form(key="query_form", clear_on_submit=True):
            ...
            with col1:
                query = st.text_area("Ask a question about your data:", key="query_input", height=100)
            with col2:
